...
export async function assessNoticeRelevance(notice:RawNotice,fetcher:typeof fetch=globalThis.fetch.bind(globalThis)):Promise<RelevanceDecision>{
  ...
  try{
    const response=await fetcher(notice.url,{headers:{"user-agent":"YouthGrant-Public-Beta/4.0 (+https://youthgrant.seoeum1711.workers.dev)"}});if(!response.ok)throw new Error(`HTTP ${response.status}`);
