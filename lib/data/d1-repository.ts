...
export async function listSourceMonitor():Promise<SourceMonitorItem[]>{
  ...
  const numberOrNull=(value:unknown)=>value===null||value===undefined?null:Number(value);return result.results.map(row=>({id:String(row.id),name:String(row.name),method:String(row.method),region:String(row.region),health:String(row.health),implemented:Boolean(row.implemented),lastRunAt:row.last_run_at?String(row.last_run_at):null,found:numberOrNull(row.items_found),itemsNew:numberOrNull(row.items_new),itemsUpdated:numberOrNull(row.items_updated),matched:numberOrNull(row.items_matched),analyzed:numberOrNull(row.items_analyzed),result:row.result?String(row.result):null,httpStatus:numberOrNull(row.http_status),parserVersion:row.parser_version?String(row.parser_version):null,error:row.error_message?String(row.error_message):null,attachmentsDiscovered:numberOrNull(row.attachments_discovered),attachmentsFetchSuccess:numberOrNull(row.attachments_fetch_success),attachmentsParseSuccess:numberOrNull(row.attachments_parse_success),attachmentsParseFailed:numberOrNull(row.attachments_parse_failed),attachmentsOcrRequired:numberOrNull(row.attachments_ocr_required),fieldsRecovered:numberOrNull(row.fields_recovered)}));
}
