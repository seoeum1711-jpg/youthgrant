...
export async function listReviewQueue():Promise<ReviewQueueItem[]>{
  ...
  const result=await db().prepare(`${reviewSelect} WHERE o.review_status IN ('PENDING','REVIEW_REQUIRED','DEFERRED') ORDER BY o.updated_at DESC LIMIT 100`).all<OpportunityRow&{raw_text:string;attachment_count:number;attachment_processed:number;attachment_failed:number}>();
