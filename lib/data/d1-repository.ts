...
export type SourceMonitorItem={id:string;name:string;method:string;region:string;health:string;implemented:boolean;enabled:boolean;lastRunAt:string|null;found:number|null;itemsNew:number|null;itemsUpdated:number|null;matched:number|null;analyzed:number|null;result:string|null;httpStatus:number|null;parserVersion:string|null;errorCode:string|null;error:string|null;attachmentsDiscovered:number|null;attachmentsFetchSuccess:number|null;attachmentsParseSuccess:number|null;attachmentsParseFailed:number|null;attachmentsOcrRequired:number|null;fieldsRecovered:number|null;discoveryComplete:number|null;discoveryUnsupported:number|null;discoveryFailed:number|null};
...
export async function listSourceMonitor():Promise<SourceMonitorItem[]>{
  const result=await db().prepare(`SELECT s.*,
    ...
    CASE WHEN sr.id IS NULL THEN NULL ELSE (SELECT COUNT(*) FROM raw_notices rn WHERE rn.source_run_id=sr.id AND rn.attachment_discovery_status='COMPLETE') END AS discovery_complete,
    CASE WHEN sr.id IS NULL THEN NULL ELSE (SELECT COUNT(*) FROM raw_notices rn WHERE rn.source_run_id=sr.id AND rn.attachment_discovery_status='UNSUPPORTED') END AS discovery_unsupported,
    CASE WHEN sr.id IS NULL THEN NULL ELSE (SELECT COUNT(*) FROM raw_notices rn WHERE rn.source_run_id=sr.id AND rn.attachment_discovery_status='FAILED') END AS discovery_failed
  ...
  const numberOrNull=(value:unknown)=>value===null||value===undefined?null:Number(value);return result.results.map(row=>{const definition=getSource(String(row.id));return{id:String(row.id),name:String(row.name),method:String(row.method),region:String(row.region),health:String(row.health),implemented:definition?.implemented??Boolean(row.implemented),enabled:definition?.enabled??false,lastRunAt:row.last_run_at?String(row.last_run_at):null,found:numberOrNull(row.items_found),itemsNew:numberOrNull(row.items_new),itemsUpdated:numberOrNull(row.items_updated),matched:numberOrNull(row.items_matched),analyzed:numberOrNull(row.items_analyzed),result:row.result?String(row.result):null,httpStatus:numberOrNull(row.http_status),parserVersion:row.parser_version?String(row.parser_version):null,errorCode:row.error_code?String(row.error_code):null,error:row.error_message?String(row.error_message):null,attachmentsDiscovered:numberOrNull(row.attachments_discovered),attachmentsFetchSuccess:numberOrNull(row.attachments_fetch_success),attachmentsParseSuccess:numberOrNull(row.attachments_parse_success),attachmentsParseFailed:numberOrNull(row.attachments_parse_failed),attachmentsOcrRequired:numberOrNull(row.attachments_ocr_required),fieldsRecovered:numberOrNull(row.fields_recovered),discoveryComplete:numberOrNull(row.discovery_complete),discoveryUnsupported:numberOrNull(row.discovery_unsupported),discoveryFailed:numberOrNull(row.discovery_failed)};});
}
