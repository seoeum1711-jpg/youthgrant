...
export class SeoulNewsApiCollector implements Collector{
  ...
  async collect(signal?:AbortSignal):Promise<RawNotice[]>{
    ...
    const endpoint=`${this.source.url}/${encodeURIComponent(this.apiKey)}/json/SeoulNewsList/1/100/`;let response:Response;try{response=await fetchWithPolicy(endpoint,{signal,headers:{"user-agent":"YouthGrant-Public-Beta/3.0 (+https://youthgrant.seoeum1711.workers.dev)"}},{fetcher:this.fetcher});}catch(error){if(error instanceof FetchPolicyError)this.lastHttpStatus=error.status;throw error;}this.lastHttpStatus=response.status;
