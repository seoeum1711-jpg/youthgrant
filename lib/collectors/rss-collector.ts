...
export class RssCollector implements Collector{
  ...
  async collect(signal?:AbortSignal):Promise<RawNotice[]>{
    const response=await this.fetcher(this.source.url,{signal,headers:{"user-agent":"YouthGrant-Public-Beta/3.0 (+https://youthgrant.seoeum1711.workers.dev)"}});this.lastHttpStatus=response.status;if(!response.ok)throw new Error(`HTTP ${response.status}`);
