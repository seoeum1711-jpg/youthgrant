...
import type { AttachmentQueueMessage, DiscoveredAttachment, DocumentRole } from "./contracts.ts";
...
function supportsDetailDiscovery(source:SourceDefinition,notice:RawNotice){return source.id==="gfgf"||source.id==="mpva"||/selectBbsNttView\.do|\/view\.do/.test(notice.url);}
...
export async function discoverAttachments(source:SourceDefinition,notice:RawNotice,fetcher:typeof fetch=globalThis.fetch.bind(globalThis)):Promise<DiscoveredAttachment[]>{
  if(!supportsDetailDiscovery(source,notice))return[];const response=await fetcher(notice.url,{headers:{"user-agent":"YouthGrant-Public-Beta/3.0 (+https://youthgrant.seoeum1711.workers.dev)"}});if(!response.ok)throw new Error(`Attachment discovery HTTP ${response.status}`);const contentType=response.headers.get("content-type")??"";if(contentType&&!/html|text/.test(contentType))return[];const html=await response.text();const found=new Map<string,DiscoveredAttachment>();
...
export async function discoverAndQueueAttachments(db:D1DatabaseLike,queue:QueueLike<AttachmentQueueMessage>|undefined,source:SourceDefinition,notice:RawNotice,rawNoticeId:string,sourceRunId:string,fetcher?:typeof fetch){
  const state=await db.prepare("SELECT attachment_discovery_status FROM raw_notices WHERE id=?").bind(rawNoticeId).first<{attachment_discovery_status:string}>();if(state?.attachment_discovery_status==="COMPLETE")return 0;let discovered:DiscoveredAttachment[];
