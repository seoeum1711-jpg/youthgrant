...
function harness(options:{run?:{id:string;status:string;startedAt:string}|null;claimed?:boolean;latest?:{id:string;status:string;startedAt:string}|null}={}){
  const messages:string[]=[];const notifications:string[]=[];let claims=0;
  return{messages,notifications,get claims(){return claims;},dependencies:{
    async findExpectedRun(_db:unknown,start:string,end:string){assert.ok(start<end);return options.run??null;},
...
test("missing expected slot sends one owner alert even when an older latest run exists",async()=>{
  const h=harness({latest:{id:"run-15",status:"SUCCESS",startedAt:"2026-08-25T06:00:50Z"}});
  const result=await runCronWatchdog(controller(slots[3]),env,h.dependencies as never);
  assert.equal(result.status,"ALERT_SENT");assert.equal(h.notifications.length,1);assert.match(h.notifications[0],/예정된 21:00 KST/);assert.match(h.notifications[0],/2026-08-25 15:00/);
});
