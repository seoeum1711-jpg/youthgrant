...
test("quality summary separates Public records from review-required records",()=>{const rows=[record(),record({id:"b",sourceId:"source-b",reviewStatus:ReviewStatus.REVIEW_REQUIRED,deadline:null,deadlineVerification:Verification.UNKNOWN,eligibilityVerification:Verification.REVIEW_REQUIRED}),record({id:"c",reviewStatus:ReviewStatus.PENDING}),record({id:"d",reviewStatus:ReviewStatus.EXCLUDED})];assert.deepEqual(summarizePublicDataQuality(rows,1),{publicTotal:1,reviewNeeded:1,deadlineVerified:1,deadlineUnknown:0,eligibilityVerified:1,eligibilityUnknown:0,duplicateCandidatePairs:1})});
