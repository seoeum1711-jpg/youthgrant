  ...
  return <>
    ...
    <div className="explorer-shell">
      ...
      <main className="results" id="grants">
        ...
        {visible.length?<div className="grant-list">{visible.map(grant=><GrantCard grant={grant} saved={savedIds.includes(grant.id)} onSave={()=>toggle(grant.id)} key={grant.id}/>)}</div>:<div className="empty-state"><h3>검색 조건에 맞는 공고가 없습니다.</h3><p>검색어나 필터를 지우면 검토 가능한 공고 {grants.length}건을 볼 수 있습니다.</p><button onClick={reset}>전체 공고 보기</button></div>}
      </main>
    </div>
...
function FilterControls({filters,setRegion,toggle,count}:{filters:ExplorerFilterState;setRegion:(value:string)=>void;toggle:(key:"facility"|"category"|"status",value:string)=>void;count:(kind:"region"|"facility"|"category"|"status",value:string)=>number}){return <><fieldset className="filter-group"><legend>지원 가능 지역</legend>{regions.map(value=><label className={filters.region===value?"selected":""} key={value}><input type="radio" name="region" checked={filters.region===value} onChange={()=>setRegion(value)}/><span>{value}</span><small>{count("region",value)}</small></label>)}</fieldset><CheckGroup title="시설유형" values={facilities} selected={filters.facility} onToggle={value=>toggle("facility",value)} count={value=>count("facility",value)}/><CheckGroup title="지원분야" values={categories} selected={filters.category} onToggle={value=>toggle("category",value)} count={value=>count("category",value)}/><CheckGroup title="접수 상태" values={statuses} selected={filters.status} onToggle={value=>toggle("status",value)} count={value=>count("status",value)}/></>}
function CheckGroup({title,values,selected,onToggle,count}:{title:string;values:readonly string[];selected:string[];onToggle:(value:string)=>void;count:(value:string)=>number}){return <fieldset className="filter-group"><legend>{title}</legend>{values.map(value=><label className={selected.includes(value)?"selected":""} key={value}><input type="checkbox" checked={selected.includes(value)} onChange={()=>onToggle(value)}/><span>{value}</span><small>{count(value)}</small></label>)}</fieldset>}

export function GrantCard({grant,saved,onSave}:{grant:GrantViewModel;saved:boolean;onSave:()=>void}){const unknown=grant.statusTone==="check"||grant.eligibilityTone!=="verified";return <article className={`grant-card ${unknown?"unknown":""}`}><Link className="card-main-link" href={`/grants/${grant.id}`} aria-label={`${grant.title} 상세 보기`}><span/></Link><div className="card-top"><span className={`status ${grant.statusTone}`}>{grant.status}</span><span className="tag">{grant.eligibleRegion}</span><span className="tag">{grant.field}</span><span className={`tag eligibility ${grant.eligibilityTone!=="verified"?"needs-review":""}`}>{grant.eligibilityLabel}</span><button type="button" className={`save-button ${saved?"saved":""}`} aria-pressed={saved} onClick={onSave}>{saved?"★ 저장됨":"☆ 저장"}</button><strong className={`dday ${!grant.dDay?"unknown-value":""}`}>{grant.dDay??"D-Day 미산출"}</strong></div><div className="card-body"><div className="card-copy"><h3>{grant.title}</h3><p className="organization">{grant.organization} · 출처 {grant.sourceName}</p><p className={`evidence ${unknown?"evidence-unknown":""}`}><strong>{grant.evidenceText}</strong></p></div><div className="summary-panel"><Summary label="지원금" value={grant.amountLabel}/><Summary label="자부담" value={grant.selfBurdenLabel}/><Summary label="신청 마감" value={grant.dateLabel}/><Link className="source-link" href={grant.sourceUrl} target="_blank" rel="noopener noreferrer">공식 공고 확인 ↗</Link></div></div></article>}
