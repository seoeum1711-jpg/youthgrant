...
export function generateStaticParams(){return fixtureOpportunities.map(({id})=>({id}));}

export async function generateMetadata({params}:{params:Promise<{id:string}>}):Promise<Metadata>{
  const {id}=await params;const row=fixtureOpportunities.find(item=>item.id===id);if(!row)return{};
...
export default async function GrantDetail({params}:{params:Promise<{id:string}>}){
  const {id}=await params;const row=fixtureOpportunities.find(item=>item.id===id);if(!row)notFound();
  const grant=toGrantViewModel(row,new Date("2026-08-21T09:00:00+09:00"));
  return <>
    <PublicHeader/>
    ...
    <div className="detail-layout">
      ...
      <aside className="detail-aside"><div className="cta-card"><a className="primary-cta" href={grant.sourceUrl} target="_blank" rel="noopener noreferrer">공식 공고 확인 ↗</a><p>{grant.sourceName} 공식 누리집으로 이동합니다.<br/>YouthGrant는 원문을 대체하지 않습니다.</p><div className="aside-summary"><Summary label="지원금" value={grant.amountLabel}/><Summary label="자부담" value={grant.selfBurdenLabel}/><Summary label="신청 마감" value={grant.dDay?`${grant.dateLabel} (${grant.dDay})`:"확인 필요"}/><Summary label="수집 방식" value={grant.sourceMethod}/><Summary label="최종 확인" value="08.21 08:40"/></div></div></aside>
    </div>
