...
export default async function GrantDetail({params}:{params:Promise<{id:string}>}){
  ...
  const deadlineSummary=grant.dDay?`${grant.dateLabel} (${grant.dDay})`:grant.status==="마감"?`${grant.dateLabel} (마감)`:"일정 확인 필요";
  return <>
    <PublicHeader/>
    ...
    <div className="detail-layout">
      <main className="detail-main">
        ...
        <DetailSection title="신청정보"><DefinitionList rows={[["신청대상",grant.eligibilityLabel],["시설유형",grant.facilityTypes.join(", ")],["신청기간",grant.applicationPeriod],["접수 상태",`${grant.status}${grant.dDay?` · ${grant.dDay}`:""}`]]}/></DetailSection>
